    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Write results mj_add
                for *xyxy, conf, cls1, cls2, cls3 in reversed(det):
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        line_1 = (cls1, *xywh, conf) if save_conf else (cls1, *xywh)  # label format
                        line_2 = (cls2, *xywh, conf) if save_conf else (cls2, *xywh)
                        line_3 = (cls3, *xywh, conf) if save_conf else (cls3, *xywh)
                        with open(f'{txt_path}.txt', 'a') as f:
                            f.write(('%g ' * len(line_1)).rstrip() % line_1 + '\n' + 
                                    ('%g ' * len(line_2)).rstrip() % line_2 + '\n' +
                                    ('%g ' * len(line_3)).rstrip() % line_3 + '\n')
